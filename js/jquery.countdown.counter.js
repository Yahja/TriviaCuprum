...
var calculateDigits = function() {
    ...
    if (secondsLeft==0) alert("Tiempo finalizado");
...
window.CounterInit = function(ticksCount) {
    ...
    for (var itemNumber = 1; itemNumber <= 5; itemNumber++) {
        ...
        if (itemNumber!=3) $element.after('<div class="digit digit0" style="margin-top: 55px"></div>');
        if (itemNumber==3) $element.after('<div class="digit digit_colon" style="margin-top: 55px"></div>');

        var $newElement = $element.next();
        $element.animate({
            "margin-top": 0
        }, 0, function () { $element.remove(); });
